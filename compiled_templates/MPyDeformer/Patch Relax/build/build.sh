#!/usr/bin/env bash
# Rebuild native plugin 'MPyDeformer_Patch_Relax' from its single source 'source/patchRelax.cpp'.
# Edit source/patchRelax.cpp, then run ./build.sh to produce MPyDeformer_Patch_Relax.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyDeformer_Patch_Relax.bundle" "$HERE/source/patchRelax.cpp"
echo "Built: $HERE/../MPyDeformer_Patch_Relax.bundle"
lipo -info "$HERE/../MPyDeformer_Patch_Relax.bundle"
