#!/usr/bin/env bash
# Rebuild native plugin 'basics_MPyDeformer' from its single source 'source/sineRippleDefault.cpp'.
# Edit source/sineRippleDefault.cpp, then run ./build.sh to produce basics_MPyDeformer.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../basics_MPyDeformer.bundle" "$HERE/source/sineRippleDefault.cpp"
echo "Built: $HERE/../basics_MPyDeformer.bundle"
lipo -info "$HERE/../basics_MPyDeformer.bundle"
