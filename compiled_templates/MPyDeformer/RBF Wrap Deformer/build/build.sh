#!/usr/bin/env bash
# Rebuild native plugin 'MPyDeformer_RBF_Wrap_Deformer' from its single source 'source/rbfWrapDeformer.cpp'.
# Edit source/rbfWrapDeformer.cpp, then run ./build.sh to produce MPyDeformer_RBF_Wrap_Deformer.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyDeformer_RBF_Wrap_Deformer.bundle" "$HERE/source/rbfWrapDeformer.cpp"
echo "Built: $HERE/../MPyDeformer_RBF_Wrap_Deformer.bundle"
lipo -info "$HERE/../MPyDeformer_RBF_Wrap_Deformer.bundle"
