#!/usr/bin/env bash
# Rebuild native plugin 'MPyFile_Game_Of_Life_Texture' from its single source 'source/gameOfLifeTex.cpp'.
# Edit source/gameOfLifeTex.cpp, then run ./build.sh to produce MPyFile_Game_Of_Life_Texture.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyFile_Game_Of_Life_Texture.bundle" "$HERE/source/gameOfLifeTex.cpp"
echo "Built: $HERE/../MPyFile_Game_Of_Life_Texture.bundle"
lipo -info "$HERE/../MPyFile_Game_Of_Life_Texture.bundle"
