#!/usr/bin/env bash
# Rebuild native plugin 'basics_MPyFile_File_Brightness_Contrast' from its single source 'source/brightContrastTex.cpp'.
# Edit source/brightContrastTex.cpp, then run ./build.sh to produce basics_MPyFile_File_Brightness_Contrast.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../basics_MPyFile_File_Brightness_Contrast.bundle" "$HERE/source/brightContrastTex.cpp"
echo "Built: $HERE/../basics_MPyFile_File_Brightness_Contrast.bundle"
lipo -info "$HERE/../basics_MPyFile_File_Brightness_Contrast.bundle"
