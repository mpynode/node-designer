#!/usr/bin/env bash
# Rebuild native plugin 'MPySkinCluster_Dual_Quaternion_Skin' from its single source 'source/dualQuaternionSkin.cpp'.
# Edit source/dualQuaternionSkin.cpp, then run ./build.sh to produce MPySkinCluster_Dual_Quaternion_Skin.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPySkinCluster_Dual_Quaternion_Skin.bundle" "$HERE/source/dualQuaternionSkin.cpp"
echo "Built: $HERE/../MPySkinCluster_Dual_Quaternion_Skin.bundle"
lipo -info "$HERE/../MPySkinCluster_Dual_Quaternion_Skin.bundle"
