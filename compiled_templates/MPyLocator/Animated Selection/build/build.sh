#!/usr/bin/env bash
# Rebuild native plugin 'MPyLocator_Animated_Selection' from its single source 'source/animatedSelection.cpp'.
# Edit source/animatedSelection.cpp, then run ./build.sh to produce MPyLocator_Animated_Selection.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyLocator_Animated_Selection.bundle" "$HERE/source/animatedSelection.cpp"
echo "Built: $HERE/../MPyLocator_Animated_Selection.bundle"
lipo -info "$HERE/../MPyLocator_Animated_Selection.bundle"
