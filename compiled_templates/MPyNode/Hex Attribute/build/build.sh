#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_Hex_Attribute' from its single source 'source/hexAttribute.cpp'.
# Edit source/hexAttribute.cpp, then run ./build.sh to produce MPyNode_Hex_Attribute.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_Hex_Attribute.bundle" "$HERE/source/hexAttribute.cpp"
echo "Built: $HERE/../MPyNode_Hex_Attribute.bundle"
lipo -info "$HERE/../MPyNode_Hex_Attribute.bundle"
