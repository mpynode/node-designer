#!/usr/bin/env bash
# Rebuild native plugin 'MPyMesh_Voxelize' from its single source 'source/voxelizeMesh.cpp'.
# Edit source/voxelizeMesh.cpp, then run ./build.sh to produce MPyMesh_Voxelize.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyMesh_Voxelize.bundle" "$HERE/source/voxelizeMesh.cpp"
echo "Built: $HERE/../MPyMesh_Voxelize.bundle"
lipo -info "$HERE/../MPyMesh_Voxelize.bundle"
