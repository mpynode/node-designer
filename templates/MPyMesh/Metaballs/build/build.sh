#!/usr/bin/env bash
# Rebuild native plugin 'MPyMesh_Metaballs' from its single source 'source/metaballs.cpp'.
# Edit source/metaballs.cpp, then run ./build.sh to produce MPyMesh_Metaballs.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyMesh_Metaballs.bundle" "$HERE/source/metaballs.cpp"
echo "Built: $HERE/../MPyMesh_Metaballs.bundle"
lipo -info "$HERE/../MPyMesh_Metaballs.bundle"
