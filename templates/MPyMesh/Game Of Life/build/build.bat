...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\gameOfLifeMesh.cpp" /Fo"%HERE%gameOfLifeMesh.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyMesh_Game_Of_Life.lib" /OUT:"%LINKTMP%\MPyMesh_Game_Of_Life.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyMesh_Game_Of_Life.mll" "%HERE%..\MPyMesh_Game_Of_Life.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%gameOfLifeMesh.obj" 2>nul
echo Built: %HERE%..\MPyMesh_Game_Of_Life.mll
exit /b 0
