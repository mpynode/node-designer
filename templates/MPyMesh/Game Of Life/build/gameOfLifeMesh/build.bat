@echo off
setlocal
REM Generated build for native node 'gameOfLifeMesh'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%gameOfLifeMesh.cpp" /Fo"%HERE%gameOfLifeMesh.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib /IMPLIB:"%LINKTMP%\gameOfLifeMesh.lib" /OUT:"%LINKTMP%\gameOfLifeMesh.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\gameOfLifeMesh.mll" "%HERE%gameOfLifeMesh.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%gameOfLifeMesh.obj" 2>nul
echo Built: %HERE%gameOfLifeMesh.mll
exit /b 0
