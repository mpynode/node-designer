@echo off
setlocal
REM Rebuild native plugin 'MPyMesh_JSON_Mesh_Reader' from its single source 'source\jsonMeshReader.cpp'.
REM Edit source\jsonMeshReader.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\jsonMeshReader.cpp" /Fo"%HERE%jsonMeshReader.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyMesh_JSON_Mesh_Reader.lib" /OUT:"%LINKTMP%\MPyMesh_JSON_Mesh_Reader.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyMesh_JSON_Mesh_Reader.mll" "%HERE%..\MPyMesh_JSON_Mesh_Reader.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%jsonMeshReader.obj" 2>nul
echo Built: %HERE%..\MPyMesh_JSON_Mesh_Reader.mll
exit /b 0
