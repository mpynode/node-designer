@echo off
REM Rebuild native plugin 'MPyMesh_Disk_Mesh_Cache' from its single source 'source\diskMeshCache.cpp'.
REM Edit source\diskMeshCache.cpp, then run build.bat from an 'x64 Native Tools Command Prompt for VS'.
REM
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\diskMeshCache.cpp" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /OUT:"%HERE%..\MPyMesh_Disk_Mesh_Cache.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
