@echo off
setlocal
REM Generated build for native node 'diskMeshCache'.
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%diskMeshCache.cpp" /Fo"%HERE%diskMeshCache.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib /IMPLIB:"%HERE%diskMeshCache.lib" /OUT:"%HERE%diskMeshCache.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%diskMeshCache.obj" "%HERE%diskMeshCache.lib" "%HERE%diskMeshCache.exp" 2>nul
echo Built: %HERE%diskMeshCache.mll
exit /b 0
