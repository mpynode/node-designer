...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%uvLayoutMesh.cpp" /Fo"%HERE%uvLayoutMesh.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib /IMPLIB:"%HERE%uvLayoutMesh.lib" /OUT:"%HERE%uvLayoutMesh.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%uvLayoutMesh.obj" "%HERE%uvLayoutMesh.lib" "%HERE%uvLayoutMesh.exp" 2>nul
