#!/usr/bin/env bash
# Rebuild native plugin 'MPyMesh_UV_Layout' from its single source 'source/uvLayoutMesh.cpp'.
# Edit source/uvLayoutMesh.cpp, then run ./build.sh to produce MPyMesh_UV_Layout.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyMesh_UV_Layout.bundle" "$HERE/source/uvLayoutMesh.cpp"
echo "Built: $HERE/../MPyMesh_UV_Layout.bundle"
lipo -info "$HERE/../MPyMesh_UV_Layout.bundle"
