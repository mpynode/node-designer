#!/usr/bin/env bash
# Rebuild native plugin 'MPyBlendShape_Combo_Correctives' from its single source 'source/comboCorrectives.cpp'.
# Edit source/comboCorrectives.cpp, then run ./build.sh to produce MPyBlendShape_Combo_Correctives.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyBlendShape_Combo_Correctives.bundle" "$HERE/source/comboCorrectives.cpp"
echo "Built: $HERE/../MPyBlendShape_Combo_Correctives.bundle"
lipo -info "$HERE/../MPyBlendShape_Combo_Correctives.bundle"
