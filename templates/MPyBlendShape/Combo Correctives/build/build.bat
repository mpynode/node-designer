@echo off
setlocal
REM Rebuild native plugin 'MPyBlendShape_Combo_Correctives' from its single source 'source\comboCorrectives.cpp'.
REM Edit source\comboCorrectives.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\comboCorrectives.cpp" /Fo"%HERE%comboCorrectives.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyBlendShape_Combo_Correctives.lib" /OUT:"%LINKTMP%\MPyBlendShape_Combo_Correctives.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyBlendShape_Combo_Correctives.mll" "%HERE%..\MPyBlendShape_Combo_Correctives.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%comboCorrectives.obj" 2>nul
echo Built: %HERE%..\MPyBlendShape_Combo_Correctives.mll
exit /b 0
