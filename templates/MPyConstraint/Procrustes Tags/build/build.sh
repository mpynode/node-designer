#!/usr/bin/env bash
# Rebuild native plugin 'MPyConstraint_Procrustes_Tags' from its single source 'source/procrustesTags.cpp'.
# Edit source/procrustesTags.cpp, then run ./build.sh to produce MPyConstraint_Procrustes_Tags.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyConstraint_Procrustes_Tags.bundle" "$HERE/source/procrustesTags.cpp"
echo "Built: $HERE/../MPyConstraint_Procrustes_Tags.bundle"
lipo -info "$HERE/../MPyConstraint_Procrustes_Tags.bundle"
