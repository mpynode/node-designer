@echo off
setlocal
REM Rebuild native plugin 'MPyConstraint_Procrustes_Tags' from its single source 'source\procrustesTags.cpp'.
REM Edit source\procrustesTags.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\procrustesTags.cpp" /Fo"%HERE%procrustesTags.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyConstraint_Procrustes_Tags.lib" /OUT:"%LINKTMP%\MPyConstraint_Procrustes_Tags.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyConstraint_Procrustes_Tags.mll" "%HERE%..\MPyConstraint_Procrustes_Tags.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%procrustesTags.obj" 2>nul
