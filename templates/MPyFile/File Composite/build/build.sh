#!/usr/bin/env bash
# Rebuild native plugin 'MPyFile_File_Composite' from its single source 'source/compositeTexture.cpp'.
# Edit source/compositeTexture.cpp, then run ./build.sh to produce MPyFile_File_Composite.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyFile_File_Composite.bundle" "$HERE/source/compositeTexture.cpp"
echo "Built: $HERE/../MPyFile_File_Composite.bundle"
lipo -info "$HERE/../MPyFile_File_Composite.bundle"
