@echo off
setlocal
REM Generated build for native node 'gameOfLifeTex'.
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%gameOfLifeTex.cpp" /Fo"%HERE%gameOfLifeTex.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaRender.lib /IMPLIB:"%HERE%gameOfLifeTex.lib" /OUT:"%HERE%gameOfLifeTex.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%gameOfLifeTex.obj" "%HERE%gameOfLifeTex.lib" "%HERE%gameOfLifeTex.exp" 2>nul
echo Built: %HERE%gameOfLifeTex.mll
exit /b 0
