...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\brightContrastTex.cpp" /Fo"%HERE%brightContrastTex.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%HERE%basics_MPyFile_File_Brightness_Contrast.lib" /OUT:"%HERE%..\basics_MPyFile_File_Brightness_Contrast.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%brightContrastTex.obj" "%HERE%basics_MPyFile_File_Brightness_Contrast.lib" "%HERE%basics_MPyFile_File_Brightness_Contrast.exp" 2>nul
