...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\brightContrastTex.cpp" /Fo"%HERE%brightContrastTex.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\basics_MPyFile_File_Brightness_Contrast.lib" /OUT:"%LINKTMP%\basics_MPyFile_File_Brightness_Contrast.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\basics_MPyFile_File_Brightness_Contrast.mll" "%HERE%..\basics_MPyFile_File_Brightness_Contrast.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%brightContrastTex.obj" 2>nul
echo Built: %HERE%..\basics_MPyFile_File_Brightness_Contrast.mll
exit /b 0
