#!/usr/bin/env bash
# Rebuild native plugin 'MPyFile_File_Simple' from its single source 'source/fileTexture.cpp'.
# Edit source/fileTexture.cpp, then run ./build.sh to produce MPyFile_File_Simple.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyFile_File_Simple.bundle" "$HERE/source/fileTexture.cpp"
echo "Built: $HERE/../MPyFile_File_Simple.bundle"
lipo -info "$HERE/../MPyFile_File_Simple.bundle"
