@echo off
setlocal
REM Generated build for native node 'fileTexture'.
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%fileTexture.cpp" /Fo"%HERE%fileTexture.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaRender.lib /IMPLIB:"%HERE%fileTexture.lib" /OUT:"%HERE%fileTexture.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%fileTexture.obj" "%HERE%fileTexture.lib" "%HERE%fileTexture.exp" 2>nul
echo Built: %HERE%fileTexture.mll
exit /b 0
