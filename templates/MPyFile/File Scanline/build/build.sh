#!/usr/bin/env bash
# Rebuild native plugin 'MPyFile_File_Scanline' from its single source 'source/scanlineTex.cpp'.
# Edit source/scanlineTex.cpp, then run ./build.sh to produce MPyFile_File_Scanline.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyFile_File_Scanline.bundle" "$HERE/source/scanlineTex.cpp"
echo "Built: $HERE/../MPyFile_File_Scanline.bundle"
lipo -info "$HERE/../MPyFile_File_Scanline.bundle"
