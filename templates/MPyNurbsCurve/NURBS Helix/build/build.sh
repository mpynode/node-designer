#!/usr/bin/env bash
# Rebuild native plugin 'MPyNurbsCurve_NURBS_Helix' from its single source 'source/helixCurve.cpp'.
# Edit source/helixCurve.cpp, then run ./build.sh to produce MPyNurbsCurve_NURBS_Helix.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNurbsCurve_NURBS_Helix.bundle" "$HERE/source/helixCurve.cpp"
echo "Built: $HERE/../MPyNurbsCurve_NURBS_Helix.bundle"
lipo -info "$HERE/../MPyNurbsCurve_NURBS_Helix.bundle"
