@echo off
setlocal
REM Rebuild native plugin 'MPyIkSolver_Two_Bone_IK' from its single source 'source\twoBoneIK.cpp'.
REM Edit source\twoBoneIK.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\twoBoneIK.cpp" /Fo"%HERE%twoBoneIK.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyIkSolver_Two_Bone_IK.lib" /OUT:"%LINKTMP%\MPyIkSolver_Two_Bone_IK.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyIkSolver_Two_Bone_IK.mll" "%HERE%..\MPyIkSolver_Two_Bone_IK.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%twoBoneIK.obj" 2>nul
