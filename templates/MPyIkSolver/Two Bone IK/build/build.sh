#!/usr/bin/env bash
# Rebuild native plugin 'MPyIkSolver_Two_Bone_IK' from its single source 'source/twoBoneIK.cpp'.
# Edit source/twoBoneIK.cpp, then run ./build.sh to produce MPyIkSolver_Two_Bone_IK.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyIkSolver_Two_Bone_IK.bundle" "$HERE/source/twoBoneIK.cpp"
echo "Built: $HERE/../MPyIkSolver_Two_Bone_IK.bundle"
lipo -info "$HERE/../MPyIkSolver_Two_Bone_IK.bundle"
