@echo off
setlocal
REM Generated build for native node 'twoBoneIK'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%twoBoneIK.cpp" /Fo"%HERE%twoBoneIK.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib /IMPLIB:"%LINKTMP%\twoBoneIK.lib" /OUT:"%LINKTMP%\twoBoneIK.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\twoBoneIK.mll" "%HERE%twoBoneIK.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%twoBoneIK.obj" 2>nul
echo Built: %HERE%twoBoneIK.mll
exit /b 0
