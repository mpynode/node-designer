...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\aimTransform.cpp" /Fo"%HERE%aimTransform.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyTransform_Aim_Between_Matrices.lib" /OUT:"%LINKTMP%\MPyTransform_Aim_Between_Matrices.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyTransform_Aim_Between_Matrices.mll" "%HERE%..\MPyTransform_Aim_Between_Matrices.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%aimTransform.obj" 2>nul
echo Built: %HERE%..\MPyTransform_Aim_Between_Matrices.mll
exit /b 0
