#!/usr/bin/env bash
# Rebuild native plugin 'MPyTransform_Aim_Between_Matrices' from its single source 'source/aimTransform.cpp'.
# Edit source/aimTransform.cpp, then run ./build.sh to produce MPyTransform_Aim_Between_Matrices.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyTransform_Aim_Between_Matrices.bundle" "$HERE/source/aimTransform.cpp"
echo "Built: $HERE/../MPyTransform_Aim_Between_Matrices.bundle"
lipo -info "$HERE/../MPyTransform_Aim_Between_Matrices.bundle"
