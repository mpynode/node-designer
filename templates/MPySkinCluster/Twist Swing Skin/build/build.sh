#!/usr/bin/env bash
# Rebuild native plugin 'MPySkinCluster_Twist_Swing_Skin' from its single source 'source/twistSwingSkin.cpp'.
# Edit source/twistSwingSkin.cpp, then run ./build.sh to produce MPySkinCluster_Twist_Swing_Skin.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPySkinCluster_Twist_Swing_Skin.bundle" "$HERE/source/twistSwingSkin.cpp"
echo "Built: $HERE/../MPySkinCluster_Twist_Swing_Skin.bundle"
lipo -info "$HERE/../MPySkinCluster_Twist_Swing_Skin.bundle"
