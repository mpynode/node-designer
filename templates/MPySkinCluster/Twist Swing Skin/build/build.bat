...
REM Edit source\twistSwingSkin.cpp, then run build.bat from an 'x64 Native Tools Command Prompt for VS'.
REM
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\twistSwingSkin.cpp" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /OUT:"%HERE%..\MPySkinCluster_Twist_Swing_Skin.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
