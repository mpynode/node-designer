...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\twistSwingSkin.cpp" /Fo"%HERE%twistSwingSkin.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPySkinCluster_Twist_Swing_Skin.lib" /OUT:"%LINKTMP%\MPySkinCluster_Twist_Swing_Skin.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPySkinCluster_Twist_Swing_Skin.mll" "%HERE%..\MPySkinCluster_Twist_Swing_Skin.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%twistSwingSkin.obj" 2>nul
echo Built: %HERE%..\MPySkinCluster_Twist_Swing_Skin.mll
exit /b 0
