@echo off
setlocal
REM Rebuild native plugin 'MPySkinCluster_Dual_Quaternion_Skin' from its single source 'source\dualQuaternionSkin.cpp'.
REM Edit source\dualQuaternionSkin.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\dualQuaternionSkin.cpp" /Fo"%HERE%dualQuaternionSkin.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPySkinCluster_Dual_Quaternion_Skin.lib" /OUT:"%LINKTMP%\MPySkinCluster_Dual_Quaternion_Skin.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPySkinCluster_Dual_Quaternion_Skin.mll" "%HERE%..\MPySkinCluster_Dual_Quaternion_Skin.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%dualQuaternionSkin.obj" 2>nul
