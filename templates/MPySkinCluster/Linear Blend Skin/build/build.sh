#!/usr/bin/env bash
# Rebuild native plugin 'MPySkinCluster_Linear_Blend_Skin' from its single source 'source/linearBlendSkin.cpp'.
# Edit source/linearBlendSkin.cpp, then run ./build.sh to produce MPySkinCluster_Linear_Blend_Skin.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPySkinCluster_Linear_Blend_Skin.bundle" "$HERE/source/linearBlendSkin.cpp"
echo "Built: $HERE/../MPySkinCluster_Linear_Blend_Skin.bundle"
lipo -info "$HERE/../MPySkinCluster_Linear_Blend_Skin.bundle"
