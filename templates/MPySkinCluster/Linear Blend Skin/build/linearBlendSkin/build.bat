@echo off
setlocal
REM Generated build for native node 'linearBlendSkin'.
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%linearBlendSkin.cpp" /Fo"%HERE%linearBlendSkin.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib /IMPLIB:"%HERE%linearBlendSkin.lib" /OUT:"%HERE%linearBlendSkin.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%linearBlendSkin.obj" "%HERE%linearBlendSkin.lib" "%HERE%linearBlendSkin.exp" 2>nul
echo Built: %HERE%linearBlendSkin.mll
exit /b 0
