#!/usr/bin/env bash
# Rebuild native plugin 'MPyNurbsSurface_NURBS_Ripple' from its single source 'source/rippleSurf.cpp'.
# Edit source/rippleSurf.cpp, then run ./build.sh to produce MPyNurbsSurface_NURBS_Ripple.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNurbsSurface_NURBS_Ripple.bundle" "$HERE/source/rippleSurf.cpp"
echo "Built: $HERE/../MPyNurbsSurface_NURBS_Ripple.bundle"
lipo -info "$HERE/../MPyNurbsSurface_NURBS_Ripple.bundle"
