...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%sineRipple.cpp" /Fo"%HERE%sineRipple.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib /IMPLIB:"%HERE%sineRipple.lib" /OUT:"%HERE%sineRipple.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%sineRipple.obj" "%HERE%sineRipple.lib" "%HERE%sineRipple.exp" 2>nul
