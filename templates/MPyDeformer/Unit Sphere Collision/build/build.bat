@echo off
setlocal
REM Rebuild native plugin 'MPyDeformer_Unit_Sphere_Collision' from its single source 'source\unitSphereCollision.cpp'.
REM Edit source\unitSphereCollision.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\unitSphereCollision.cpp" /Fo"%HERE%unitSphereCollision.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyDeformer_Unit_Sphere_Collision.lib" /OUT:"%LINKTMP%\MPyDeformer_Unit_Sphere_Collision.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyDeformer_Unit_Sphere_Collision.mll" "%HERE%..\MPyDeformer_Unit_Sphere_Collision.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%unitSphereCollision.obj" 2>nul
echo Built: %HERE%..\MPyDeformer_Unit_Sphere_Collision.mll
exit /b 0
