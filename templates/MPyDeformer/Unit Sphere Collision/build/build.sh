#!/usr/bin/env bash
# Rebuild native plugin 'MPyDeformer_Unit_Sphere_Collision' from its single source 'source/unitSphereCollision.cpp'.
# Edit source/unitSphereCollision.cpp, then run ./build.sh to produce MPyDeformer_Unit_Sphere_Collision.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyDeformer_Unit_Sphere_Collision.bundle" "$HERE/source/unitSphereCollision.cpp"
echo "Built: $HERE/../MPyDeformer_Unit_Sphere_Collision.bundle"
lipo -info "$HERE/../MPyDeformer_Unit_Sphere_Collision.bundle"
