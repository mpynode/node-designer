@echo off
setlocal
REM Rebuild native plugin 'basics_MPyDeformer' from its single source 'source\sineRippleDefault.cpp'.
REM Edit source\sineRippleDefault.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\sineRippleDefault.cpp" /Fo"%HERE%sineRippleDefault.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\basics_MPyDeformer.lib" /OUT:"%LINKTMP%\basics_MPyDeformer.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\basics_MPyDeformer.mll" "%HERE%..\basics_MPyDeformer.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%sineRippleDefault.obj" 2>nul
echo Built: %HERE%..\basics_MPyDeformer.mll
exit /b 0
