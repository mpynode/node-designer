...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\rbfWrapDeformer.cpp" /Fo"%HERE%rbfWrapDeformer.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%HERE%MPyDeformer_RBF_Wrap_Deformer.lib" /OUT:"%HERE%..\MPyDeformer_RBF_Wrap_Deformer.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%rbfWrapDeformer.obj" "%HERE%MPyDeformer_RBF_Wrap_Deformer.lib" "%HERE%MPyDeformer_RBF_Wrap_Deformer.exp" 2>nul
echo Built: %HERE%..\MPyDeformer_RBF_Wrap_Deformer.mll
exit /b 0
