...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%rbfWrapDeformer.cpp" /Fo"%HERE%rbfWrapDeformer.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib /IMPLIB:"%LINKTMP%\rbfWrapDeformer.lib" /OUT:"%LINKTMP%\rbfWrapDeformer.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\rbfWrapDeformer.mll" "%HERE%rbfWrapDeformer.mll" >nul
if errorlevel 1 exit /b 1
