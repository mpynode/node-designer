@echo off
setlocal
REM Generated build for native node 'circularText'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" /Zc:__cplusplus /permissive- /I "%QTINC%" /FI nd_msvc_stdext_compat.h "%HERE%circularText.cpp" /Fo"%HERE%circularText.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Qt6Core.lib Qt6Gui.lib Qt6Widgets.lib /IMPLIB:"%LINKTMP%\circularText.lib" /OUT:"%LINKTMP%\circularText.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\circularText.mll" "%HERE%circularText.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%circularText.obj" 2>nul
echo Built: %HERE%circularText.mll
exit /b 0
