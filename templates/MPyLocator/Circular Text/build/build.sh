#!/usr/bin/env bash
# Rebuild native plugin 'MPyLocator_Circular_Text' from its single source 'source/circularText.cpp'.
# Edit source/circularText.cpp, then run ./build.sh to produce MPyLocator_Circular_Text.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyLocator_Circular_Text.bundle" "$HERE/source/circularText.cpp"
echo "Built: $HERE/../MPyLocator_Circular_Text.bundle"
lipo -info "$HERE/../MPyLocator_Circular_Text.bundle"
