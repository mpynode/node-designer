@echo off
setlocal
REM Generated build for native node 'animatedText'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" /Zc:__cplusplus /permissive- /I "%QTINC%" /FI nd_msvc_stdext_compat.h "%HERE%animatedText.cpp" /Fo"%HERE%animatedText.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Qt6Core.lib Qt6Gui.lib Qt6Widgets.lib /IMPLIB:"%LINKTMP%\animatedText.lib" /OUT:"%LINKTMP%\animatedText.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\animatedText.mll" "%HERE%animatedText.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%animatedText.obj" 2>nul
echo Built: %HERE%animatedText.mll
exit /b 0
