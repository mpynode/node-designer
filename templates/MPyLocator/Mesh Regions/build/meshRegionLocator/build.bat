@echo off
setlocal
REM Generated build for native node 'meshRegionLocator'.
...
set "HERE=%~dp0"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" /Zc:__cplusplus /permissive- /I "%QTINC%" /FI nd_msvc_stdext_compat.h "%HERE%meshRegionLocator.cpp" /Fo"%HERE%meshRegionLocator.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Qt6Core.lib Qt6Gui.lib Qt6Widgets.lib /IMPLIB:"%HERE%meshRegionLocator.lib" /OUT:"%HERE%meshRegionLocator.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
del "%HERE%meshRegionLocator.obj" "%HERE%meshRegionLocator.lib" "%HERE%meshRegionLocator.exp" 2>nul
echo Built: %HERE%meshRegionLocator.mll
exit /b 0
