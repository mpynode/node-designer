@echo off
setlocal
REM Generated build for native node 'widgetShowcase'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%widgetShowcase.cpp" /Fo"%HERE%widgetShowcase.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib /IMPLIB:"%LINKTMP%\widgetShowcase.lib" /OUT:"%LINKTMP%\widgetShowcase.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\widgetShowcase.mll" "%HERE%widgetShowcase.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%widgetShowcase.obj" 2>nul
echo Built: %HERE%widgetShowcase.mll
exit /b 0
