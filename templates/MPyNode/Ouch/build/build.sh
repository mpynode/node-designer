#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_Ouch' from its single source 'source/ouch.cpp'.
# Edit source/ouch.cpp, then run ./build.sh to produce MPyNode_Ouch.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_Ouch.bundle" "$HERE/source/ouch.cpp"
echo "Built: $HERE/../MPyNode_Ouch.bundle"
lipo -info "$HERE/../MPyNode_Ouch.bundle"
