#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_DNET' from its single source 'source/mPyDnet.cpp'.
# Edit source/mPyDnet.cpp, then run ./build.sh to produce MPyNode_DNET.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_DNET.bundle" "$HERE/source/mPyDnet.cpp"
echo "Built: $HERE/../MPyNode_DNET.bundle"
lipo -info "$HERE/../MPyNode_DNET.bundle"
