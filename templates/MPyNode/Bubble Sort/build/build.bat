@echo off
setlocal
REM Rebuild native plugin 'MPyNode_Bubble_Sort' from its single source 'source\bubbleSort.cpp'.
REM Edit source\bubbleSort.cpp, then run build.bat from any cmd.exe -- it sets up MSVC itself.
REM
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%source\bubbleSort.cpp" /Fo"%HERE%bubbleSort.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib OpenMayaAnim.lib OpenMayaUI.lib OpenMayaRender.lib Foundation.lib /IMPLIB:"%LINKTMP%\MPyNode_Bubble_Sort.lib" /OUT:"%LINKTMP%\MPyNode_Bubble_Sort.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\MPyNode_Bubble_Sort.mll" "%HERE%..\MPyNode_Bubble_Sort.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%bubbleSort.obj" 2>nul
echo Built: %HERE%..\MPyNode_Bubble_Sort.mll
exit /b 0
