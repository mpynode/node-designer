@echo off
setlocal
REM Generated build for native node 'bubbleSort'.
...
mkdir "%LINKTMP%"
cl /nologo /LD /std:c++17 /O2 /fp:precise /EHsc /MD /bigobj /utf-8 /D NT_PLUGIN /D REQUIRE_IOSTREAM /D _BOOL /D WIN32 /D _WINDOWS /D _CRT_SECURE_NO_WARNINGS /I "%MAYA%\include" "%HERE%bubbleSort.cpp" /Fo"%HERE%bubbleSort.obj" /link /LIBPATH:"%MAYA%\lib" OpenMaya.lib Foundation.lib /IMPLIB:"%LINKTMP%\bubbleSort.lib" /OUT:"%LINKTMP%\bubbleSort.mll" /EXPORT:initializePlugin /EXPORT:uninitializePlugin
if errorlevel 1 exit /b 1
copy /Y "%LINKTMP%\bubbleSort.mll" "%HERE%bubbleSort.mll" >nul
if errorlevel 1 exit /b 1
rd /s /q "%LINKTMP%" 2>nul
del "%HERE%bubbleSort.obj" 2>nul
echo Built: %HERE%bubbleSort.mll
exit /b 0
