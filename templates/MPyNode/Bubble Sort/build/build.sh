#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_Bubble_Sort' from its single source 'source/bubbleSort.cpp'.
# Edit source/bubbleSort.cpp, then run ./build.sh to produce MPyNode_Bubble_Sort.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_Bubble_Sort.bundle" "$HERE/source/bubbleSort.cpp"
echo "Built: $HERE/../MPyNode_Bubble_Sort.bundle"
lipo -info "$HERE/../MPyNode_Bubble_Sort.bundle"
