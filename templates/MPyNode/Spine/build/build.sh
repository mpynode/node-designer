#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_Spine' from its single source 'source/spine.cpp'.
# Edit source/spine.cpp, then run ./build.sh to produce MPyNode_Spine.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_Spine.bundle" "$HERE/source/spine.cpp"
echo "Built: $HERE/../MPyNode_Spine.bundle"
lipo -info "$HERE/../MPyNode_Spine.bundle"
