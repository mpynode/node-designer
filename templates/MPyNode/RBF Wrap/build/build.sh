#!/usr/bin/env bash
# Rebuild native plugin 'MPyNode_RBF_Wrap' from its single source 'source/rbfWrap.cpp'.
# Edit source/rbfWrap.cpp, then run ./build.sh to produce MPyNode_RBF_Wrap.bundle (one level up).
#
...
clang++ -std=c++17 -O3 -ffp-contract=off -arch arm64 -bundle \
  ...
  -o "$HERE/../MPyNode_RBF_Wrap.bundle" "$HERE/source/rbfWrap.cpp"
echo "Built: $HERE/../MPyNode_RBF_Wrap.bundle"
lipo -info "$HERE/../MPyNode_RBF_Wrap.bundle"
