...
class MAngle(om.MAngle):
    ...
    def __repr__(self):
        return super().__repr__().replace("maya.api.OpenMaya.", "MPyNode.")
...
class MColor(om.MColor):
    ...
    def __init__(self, clr=None, model=om.MColor.kRGB, dataType=om.MColor.kFloat):
        ...
        if clr:
            super().__init__(clr, model=model, dataType=dataType)
    ...
    def __reduce__(self):
        return (
            MColor,
            ((self.r, self.g, self.b, self.a), self._model, self._data_type),
        )
    ...
    def __add__(self, y):
        return MColor(super().__add__(y))

    def __radd__(self, y):
        return MColor(super().__radd__(y))

    def __iadd__(self, y):
        return MColor(super().__iadd__(y))

    def __mul__(self, y):
        return MColor(super().__mul__(y))

    def __rmul__(self, y):
        return MColor(super().__rmul__(y))

    def __imul__(self, y):
        return MColor(super().__imul__(y))

    def __div__(self, y):
        return MColor(super().__div__(y))

    def __idiv__(self, y):
        return MColor(super().__idiv__(y))

    def __rdiv__(self, y):
        return MColor(super().__rdiv__(y))
    ...
    def setColor(self, clr, model=om.MColor.kRGB, dataType=om.MColor.kFloat):
        ...
        super().setColor(clr, model=model, dataType=dataType)
...
class MColorArray(om.MColorArray):
    ...
    def __getitem__(self, i):
        return MColor(super().__getitem__(i))

    def __getslice__(self, i, j):
        return MColorArray(super().__getslice__(i, j))

    def __repr__(self):
        return super().__repr__().replace("maya.api.OpenMaya.", "MPyNode.")

    def __str__(self):
        return super().__str__().replace("maya.api.OpenMaya.", "MPyNode.")
...
class MEulerRotation(om.MEulerRotation):
    ...
    def __add__(self, y):
        return MEulerRotation(super().__add__(y))

    def __radd__(self, y):
        return MEulerRotation(super().__radd__(y))

    def __iadd__(self, y):
        return MEulerRotation(super().__iadd__(y))

    def __sub__(self, y):
        return MEulerRotation(super().__sub__(y))

    def __rsub__(self, y):
        return MEulerRotation(super().__rsub__(y))

    def __isub__(self, y):
        return MEulerRotation(super().__isub__(y))

    def __mul__(self, y):
        return MEulerRotation(super().__mul__(y))

    def __rmul__(self, y):
        return MEulerRotation(super().__rmul__(y))

    def __imul__(self, y):
        return MEulerRotation(super().__imul__(y))

    def alternateSolution(self):
        return MEulerRotation(super().alternateSolution())

    def asMatrix(self):
        return MMatrix(super().asMatrix())

    def asQuaternion(self):
        return MQuaternion(super().asQuaternion())

    def asVector(self):
        return MVector(super().asVector())

    def bound(self):
        return MEulerRotation(super().bound())

    def closestCut(self, target):
        return MEulerRotation(super().closestCut(target))

    def closestSolution(self, target):
        return MEulerRotation(super().closestSolution(target))
    ...
    def inverse(self):
        return MEulerRotation(super().inverse())

    def reorder(self, order):
        return MEulerRotation(super().reorder(order))
...
class MFnMesh(om.MFnMesh):
    ...
    def getPoints(self, space=om.MSpace.kObject):
        return MPointArray(super().getPoints(space))
...
class MFnNurbsCurve(om.MFnNurbsCurve):
    ...
    def closestPoint(
        ...
        pnt, param = super().closestPoint(test_pnt, guess, tolerance, space)

        return MPoint(pnt), param

    def cvPosition(self, index, space=om.MSpace.kObject):
        return MPoint(super().cvPosition(index, space))

    def cvPositions(self, space=om.MSpace.kObject):
        return MPointArray(super().cvPositions(space))

    def getDerivativesAtParam(self, param, space=om.MSpace.kObject, dUU=False):
        result = super().getDerivativesAtParam(param, space=space, dUU=dUU)
    ...
    def getPointAtParam(self, param, space=om.MSpace.kObject):
        return MPoint(super().getPointAtParam(param, space))

    def normal(self, param, space=om.MSpace.kObject):
        return MVector(super().normal(param, space))

    def tangent(self, param, space=om.MSpace.kObject):
        return MVector(super().tangent(param, space))
...
class MMatrix(om.MMatrix):
    ...
    def __add__(self, y):
        return MMatrix(super().__add__(y))

    def __radd__(self, y):
        return MMatrix(super().__radd__(y))

    def __iadd__(self, y):
        return MMatrix(super().__iadd__(y))

    def __sub__(self, y):
        return MMatrix(super().__sub__(y))

    def __rsub__(self, y):
        return MMatrix(super().__rsub__(y))

    def __isub__(self, y):
        return MMatrix(super().__isub__(y))

    def __mul__(self, y):
        result = super().__mul__(y)
    ...
    def __rmul__(self, y):
        return MMatrix(super().__rmul__(y))

    def __imul__(self, y):
        return MMatrix(super().__imul__(y))
    ...
    def homogenize(self):
        ...
        return MMatrix(super().homogenize())
...
class MMatrixArray(om.MMatrixArray):
    ...
    def __getitem__(self, i):
        return MMatrix(super().__getitem__(i))

    def __getslice__(self, i, j):
        return MMatrixArray(super().__getslice__(i, j))
...
class MPoint(om.MPoint):
    ...
    def __add__(self, y):
        return MPoint(super().__add__(y))

    def __radd__(self, y):
        return MPoint(super().__radd__(y))

    def __iadd__(self, y):
        return MPoint(super().__iadd__(y))

    def __mul__(self, y):
        return MPoint(super().__mul__(y))

    def __rmul__(self, y):
        return MPoint(super().__rmul__(y))

    def __imul__(self, y):
        return MPoint(super().__imul__(y))

    def __div__(self, y):
        return MPoint(super().__div__(y))

    def __rdiv__(self, y):
        return MPoint(super().__rdiv__(y))

    def __sub__(self, y):
        diff = super().__sub__(y)

        if type(diff) is om.MVector:
            return MVector(diff)

        return MPoint(diff)

    def __rsub__(self, y):
        diff = super().__rsub__(y)
    ...
    def __isub__(self, y):
        ...
        return MPoint(super().__isub__(y))
...
class MPointArray(om.MPointArray):
    ...
    def __iadd__(self, y):
        return MPointArray(super().__iadd__(y))

    def __getitem__(self, i):
        return MPoint(super().__getitem__(i))

    def __getslice__(self, i, j):
        return MPointArray(super().__getslice__(i, j))
...
class MQuaternion(om.MQuaternion):
    ...
    def __add__(self, y):
        return MQuaternion(super().__add__(y))

    def __radd__(self, y):
        return MQuaternion(super().__radd__(y))

    def __sub__(self, y):
        return MQuaternion(super().__sub__(y))

    def __rsub__(self, y):
        return MQuaternion(super().__rsub__(y))

    def __mul__(self, y):
        return MQuaternion(super().__mul__(y))

    def __rmul__(self, y):
        return MQuaternion(super().__rmul__(y))

    def __imul__(self, y):
        return MQuaternion(super().__imul__(y))
    ...
    def __neg__(self):
        ...
        return MQuaternion(super().__neg__())
    ...
    def asEulerRotation(self):
        ...
        return MEulerRotation(super().asEulerRotation())
    ...
    def conjugate(self):
        ...
        return MQuaternion(super().conjugate())
    ...
    def inverse(self):
        ...
        return MQuaternion(super().inverse())
    ...
    def normal(self):
        ...
        return MQuaternion(super().normal())
...
class MTimeArray(om.MTimeArray):
    ...
    def __getitem__(self, i):
        return MTime(super().__getitem__(i))

    def __getslice__(self, i, j):
        return MTimeArray(super().__getslice__(i, j))
...
class MVector(om.MVector):
    ...
    def __add__(self, y):
        return MVector(super().__add__(y))

    def __radd__(self, y):
        return MVector(super().__radd__(y))

    def __iadd__(self, y):
        return MVector(super().__iadd__(y))

    def __sub__(self, y):
        return MVector(super().__sub__(y))

    def __rsub__(self, y):
        return MVector(super().__rsub__(y))
    ...
    def __isub__(self, y):
        ...
        return MVector(super().__isub__(y))
    ...
    def __xor__(self, y):
        ...
        return MVector(super().__xor__(y))
    ...
    def __rxor__(self, y):
        ...
        return MVector(super().__rxor__(y))

    def __mul__(self, y):
        result = super().__mul__(y)

        if type(result) is om.MVector:
            return MVector(result)

        return result

    def __rmul__(self, y):
        result = super().__rmul__(y)
    ...
    def __div__(self, y):
        return MVector(super().__div__(y))

    def __idiv__(self, y):
        return MVector(super().__idiv__(y))

    def __rdiv__(self, y):
        return MVector(super().__rdiv__(y))
    ...
    def normalize(self):
        ...
        **RETURNS** 	*MVector*
        ...
        return MVector(super().normalize())
    ...
    def rotateBy(self, *args):
        ...
        **RETURNS** 	*MVector*
        ...
        return MVector(super().rotateBy(*args))
    ...
    def rotateTo(self, target):
        ...
        **RETURNS** 	*MQuaternion*
        ...
        return MQuaternion(super().rotateTo(target))
    ...
    def transformAsNormal(self, matrix):
        ...
        **RETURNS**		*MVector*
        ...
        return MVector(super().transformAsNormal(matrix))
...
class MVectorArray(om.MVectorArray):
    ...
    def __getitem__(self, i):
        return MVector(super().__getitem__(i))

    def __getslice__(self, i, j):
        return MVectorArray(super().__getslice__(i, j))
    ...
    def __iadd__(self, y):
        return MVectorArray(super().__iadd__(y))
