...
from mpylib._mpynode._openmaya import (
    MAngle,
    MColor,
    MColorArray,
    MEulerRotation,
...
class MPyNode(om.MPxNode):
    ...
    ATTR_TYPES_MAP = {
        INT_ATTR_TYPE: ("_getIntInput", "_setIntOutput", DEFAULT_INT, INT_LIST_TYPE),
        FLOAT_ATTR_TYPE: (
            "_getFloatInput",
            "_setFloatOutput",
            DEFAULT_FLOAT,
        ...
        NURBS_CURVE_TYPE: (
            ...
            None,
            DEFAULT_NURBS_CURVE,
            NURBS_CURVE_LIST_TYPE,
        ),
        NURBS_SURFACE_TYPE: (
            "_getNurbsSurfaceInput",
            None,
            DEFAULT_NURBS_SURFACE,
            NURBS_SURFACE_LIST_TYPE,
        ),
        PY_ATTR_TYPE: (
            "_getPythonInput",
            "_setPythonOutput",
    ...
    def compute(self, plug, data_block):
        ...
        if self.run_profiler:
            ...
            om.MUserEventMessage.postUserEvent(
                self.PROFILE_CALLBACK_NAME, (self.thisMObject(), profiler.getstats())
            )
        ...
        if self._watch_var_values:
            om.MUserEventMessage.postUserEvent(
                self.VAR_VALUES_CALLABCK_NAME, (self.thisMObject(), self._exec_vars)
            )
    ...
    def print_(self, txt):
        om.MUserEventMessage.postUserEvent(
            self.LOG_TXT_CALLBACK_NAME, (self.thisMObject(), str(txt))
        )
    ...
    def _getInitOutputValues(self):
        ...
        for attr_name, attr_data in self._output_plug_map.items():
            ...
            if attr_data[3]:
                outputs[attr_name] = attr_data[4](
                    [attr_data[2]] * attr_data[0].numElements()
                )
            ...
            for attr_name, plug_data in self._output_plug_map.items():
                if attr_name in self._exec_vars:
                    ...
                    plug_data[1](
                        data_block,
                        plug_data[0],
                        self._exec_vars[attr_name],
                        is_array=plug_data[3],
                    )
    ...
    def _getInputValues(self, data_block):
        ...
        if self._input_plug_map:
            for attr_name, plug_data in self._input_plug_map.items():
                inputs[attr_name] = plug_data[1](
                    data_block, plug_data[0], is_array=plug_data[2]
                )  ## pass the data handle to the proper _get method
    ...
    def setDependentsDirty(self, plug, affected_plugs):
        ...
        plug_name = (
            plug.partialName() if not plug.isChild else plug.parent().partialName()
        )

        ##---look for the attribute name in the input map...handling array attr syntax as needed---##
        if (plug_name.split("[")[0] in self._input_plug_map) or (
            plug == self.__class__._expression_plug
        ):
                ...
                if out_plug.isArray and out_plug.numElements() > 0:
                    for e in range(out_plug.numElements()):
                        ...
                        if e_plug.isCompound:
                            map(
                                affected_plugs.append,
                                map(e_plug.child, range(e_plug.numChildren())),
                            )

                elif (not out_plug.isArray) and out_plug.isCompound:
                    map(
                        affected_plugs.append,
                        map(out_plug.child, range(out_plug.numChildren())),
                    )
            ...
            if attr_dict:
                for attr_name, attr_properties in attr_dict.items():
                    if self._node_fn.hasAttribute(attr_name):
                        ...
                        attr_type_map = self.ATTR_TYPES_MAP[
                            attr_properties[0]
                        ]  ##get proper dict based on attribute type
                        get_func_name = attr_type_map[
                            0
                        ]  ##get the string name of the function to 'get' values for this attribute

                        ##----value indices are (MPlug of this plug, the 'get' function, bool whether this if an array attribute)
                        self._input_plug_map[plug.partialName()] = (
                            plug,
                            getattr(self, get_func_name),
                            plug.isArray,
                        )
            ...
            if attr_dict:
                for attr_name, attr_properties in attr_dict.items():
                    if self._node_fn.hasAttribute(attr_name):
                        ...
                        set_func_name = attr_type_map[
                            1
                        ]  ##get the string name of the function to 'set' values for this attribute
                        default_val = (
                            attr_type_map[2]() if attr_type_map[2] is not None else None
                        )  ##get default value for this plug type
                        ...
                        self._output_plug_map[plug.partialName()] = (
                            plug,
                            getattr(self, set_func_name),
                            default_val,
                            plug.isArray,
                            array_type,
                        )
            ...
            if attr_dict:
                for attr_name, attr_properties in attr_dict.items():
                    if self._node_fn.hasAttribute(attr_name):
                        ...
                        default_val = attr_type_map[
                            2
                        ]()  ##get default value for this plug type
                        array_type = attr_type_map[3]

                        ##----value indices are (MPlug of this plug, the 'set' function, default value for the plug, bool whether this if an array attribute)
                        self._stored_var_map[plug.partialName()] = (
                            plug,
                            getattr(self, set_func_name),
                            default_val,
                            plug.isArray,
    ...
    def _execExpression(self):
        ...
        if self._exp_code_obj:
            try:
                ...
                exec(
                    self._exp_code_obj, self._exec_vars, self._exec_vars
                )  # fixes nested vars and namepaces

            except:
                except_data = sys.exc_info()
                tb_lines = traceback.format_exception(
                    except_data[0], except_data[1], except_data[2]
                )
                err_str = (
                ...
                om.MUserEventMessage.postUserEvent(
                    self.LOG_ERROR_CALLBACK_NAME, (self.thisMObject(), err_str)
                )

                print(err_str)

    def _getGenericInput(
        self,
        data_block,
        plug,
        data_class,
    ...
    def _getFloatInput(self, data_block, plug, is_array=False):
        ...
        """
        return self._getGenericInput(
            data_block,
            plug,
            float,
            "asDouble",
            is_array=is_array,
            array_type=self.FLOAT_LIST_TYPE,
        )
    ...
    def _getEnumInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            data_block, plug, int, "asShort", is_array=is_array
        )
    ...
    def _getBoolInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            ...
            "asBool",
            is_array=is_array,
            array_type=self.BOOL_LIST_TYPE,
        )
    ...
    def _getVectorInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            ...
            "asVector",
            is_array=is_array,
            array_type=self.VECTOR_LIST_TYPE,
        )
    ...
    def _getAngleInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            ...
            "asAngle",
            is_array=is_array,
            array_type=self.ANGLE_LIST_TYPE,
        )
    ...
    def _getStringInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            data_block, plug, str, "asString", is_array=is_array
        )
    ...
    def _getGeoInput(
        self, data_block, plug, get_func_name, fn_class, is_array=False, array_type=list
    ):
        """
    ...
    def _getMatrixInput(self, data_block, plug, is_array=False):
        ...
        return self._getGenericInput(
            ...
            "asMatrix",
            is_array=is_array,
            array_type=self.MATRIX_LIST_TYPE,
        )
    ...
    def _setVectorOutput(self, data_block, plug, val, is_array=False):
        ...
        if val is not None:
            if not is_array:
                out_handle = data_block.outputValue(plug)
                out_handle.set3Double(
                    val[0], val[1], val[2]
                )  # allows lists len > 3 to be used
                out_handle.setClean()
    ...
    def _loadPickle(self, data):
        ...
        try:
            try:
                ...
                return pickle.loads(
                    str(data)
                )  # backwards compatibility with default pickle protocol
        except:
            pass
    ...
    def _baseInitializer(cls):
        ...
        cls._expression_plug = exp_attr.create(
            cls.EXPRESSION_ATTR_NAME,
            cls.EXPRESSION_ATTR_SHORT_NAME,
            om.MFnStringData.kString,
            exp_str_creator,
        )
        ...
        cls._in_attrs_str_plug = in_data_attr.create(
            cls._INPUTS_STR_ATTR_NAME,
            cls._INPUTS_STR_ATTR_NAME,
            om.MFnStringData.kString,
            in_str_creator,
        )
        ...
        cls._out_attrs_str_plug = out_data_attr.create(
            cls._OUTPUTS_STR_ATTR_NAME,
            cls._OUTPUTS_STR_ATTR_NAME,
            om.MFnStringData.kString,
            out_str_creator,
        )
        ...
        cls._stored_var_names_plug = var_list_attr.create(
            cls._STORED_VARS_ATTR_NAME,
            cls._STORED_VARS_ATTR_NAME,
            om.MFnStringData.kString,
            var_list_str_creator,
        )
        ...
        cls._stored_var_data_plug = var_vals_attr.create(
            cls._STORED_VARS_DATA_ATTR_NAME,
            cls._STORED_VARS_DATA_ATTR_NAME,
            om.MFnStringData.kString,
            var_vals_str_creator,
        )
...
class MPyNodeEventManager(object):
    ...
    def startCallbacks(self):
        ...
        **RETURNS**		*None*
        ...
        self._scene_open_cb_id = om.MSceneMessage.addCallback(
            om.MSceneMessage.kBeforeOpen, self._onBeforeSceneOpened, None
        )
        self._scene_read_cb_id = om.MSceneMessage.addCallback(
            om.MSceneMessage.kAfterFileRead, self._onPostSceneRead, None
        )
        self._scene_new_cb_id = om.MSceneMessage.addCallback(
            om.MSceneMessage.kBeforeNew, self._onBeforeSceneOpened, None
        )
        self._scene_write_cb_id = om.MSceneMessage.addCallback(
            om.MSceneMessage.kBeforeSave, self._onSceneSaved, None
        )

        self._node_added_cb_id = om.MDGMessage.addNodeAddedCallback(
            self._onNodeAdded, MPyNode.NODE_NAME, None
        )
        self._node_removed_cb_id = om.MDGMessage.addNodeRemovedCallback(
            self._onNodeRemoved, MPyNode.NODE_NAME, None
        )

        self._set_var_cb_id = om.MUserEventMessage.addUserEventCallback(
            self.SET_VAR_CALLBACK_NAME, self._onSetStoredVariable, None
        )
        self._write_var_cb_id = om.MUserEventMessage.addUserEventCallback(
            self.WRITE_STORED_VARS_CALLBACK_NAME, self._onWriteStoredVariables, None
        )
    ...
    def _onNodeRemoved(self, m_obj, data):
        ...
        if not m_obj.isNull():
            ...
            if hash_code in self._py_nodes:
                if hash_code not in self._py_nodes_removed:
    ...
    def _onSetStoredVariable(self, data):
        ...
        for node_hash, var_vals in data.items():
            if node_hash in self._py_nodes:
                ...
                raise RuntimeError(
                    "Unable to set storable variable data for node: "
                    + str(node_hash)
                    + " = "
                    + str(var_vals)
                )
    ...
    def _onWriteStoredVariables(self, data):
        ...
        for node_hash in data:
            if node_hash in self._py_nodes and self._py_nodes[node_hash].isValid():
                ...
                raise RuntimeError(
                    "Unable to write storable variables for node: " + str(node_hash)
                )
    ...
    def addNode(self, py_node):
        ...
        **RETURNS**		*None*
        ...
        if hash_code not in self._py_nodes:
