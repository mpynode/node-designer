...
_LOCATOR_INCLUDES_PLUGIN = [
    ...
    "maya/MFrameContext.h", "maya/MViewport2Renderer.h", "maya/MDrawContext.h",
...
def _locator_hover_service(cls, data_cls, svars, needs_hover=True):
    ...
    if needs_hover:
        ...
        L.append("static double        g_lastRedraw = 0.0;   // measured cost (s) of the last requested redraw")
        L.append("static bool          g_costPending = false; // a request is out; the first end-of-render after it measures the cost")
        L.append("static bool          g_notifOn = false;    // VP2 end-of-render notification registered")
    ...
    if needs_hover:
        ...
        L.append("}")
        L.append("static void _onEndRender(MHWRender::MDrawContext&, void*) {")
        L.append("    // VP2 finished a render. If our idle-redraw request was still unpaid, the")
        L.append("    // time since the request is what the main thread spent servicing it:")
        L.append("    // evaluation, prepareForDraw and the draw itself.")
        L.append("    if (g_costPending) { g_lastRedraw = std::max(0.0, _wallClock() - g_lastDirtyT); g_costPending = false; }")
        L.append("}")
    ...
    if needs_hover:
        ...
        L.append("    // keeps animating after the cursor leaves the gizmo), THROTTLED BY")
        L.append("    // MEASUREMENT: a request notes the time, and VP2's end-of-render")
        L.append("    // notification (_onEndRender) turns the first render after it into the")
        L.append("    // request's cost. The next request waits 2x that cost (never less than a")
        L.append("    // tick) and none goes out while one is still unpaid, so idle animation")
        L.append("    // takes at most ~half the main thread however many locators are in the")
        L.append("    // scene (unthrottled, 100 nodes saturated it at 7.8 fps forever) and a")
        L.append("    // cheap redraw keeps the full tick rate. Inferring the cost from tick")
        L.append("    // lateness misread Maya's own ~47 ms tick cadence and missed redraws")
        L.append("    // that ran before the due tick. Port of draw_refresh.throttle_step.")
        L.append("    const double _now = _wallClock(), _period = 1.0 / 30.0;")
        L.append("    if (g_costPending && (_now - g_lastDirtyT) > 1.0) g_costPending = false;   // render never came (hidden viewport)")
        L.append("    const double _wait = std::max(_period, 2.0 * g_lastRedraw);")
        L.append("    if (!g_autoRefresh.empty() && !g_costPending && (g_lastDirtyT < 0.0 || (_now - g_lastDirtyT) >= _wait)) {")
        L.append("        g_lastDirtyT = _now; g_costPending = g_notifOn;")
    ...
    if needs_hover:
        ...
        L.append("    g_costPending = false; g_lastDirtyT = -1.0; g_lastRedraw = 0.0;")
    ...
    if needs_hover:
        ...
        L.append("    if (s && g_timerId) {")
        L.append("        MHWRender::MRenderer* _r = MHWRender::MRenderer::theRenderer();")
        L.append("        g_notifOn = (_r && _r->addNotification(_onEndRender, \"%sIdleRefreshCost\","
                 % cls)
        L.append("                     MHWRender::MPassContext::kEndRenderSemantic, NULL) == MS::kSuccess);")
        L.append("        g_started = true; return;")
        L.append("    }")
    ...
    if needs_hover:
        ...
        L.append("    if (g_notifOn) { MHWRender::MRenderer* _r = MHWRender::MRenderer::theRenderer();")
        L.append("                     if (_r) _r->removeNotification(\"%sIdleRefreshCost\", MHWRender::MPassContext::kEndRenderSemantic);"
                 % cls)
        L.append("                     g_notifOn = false; }")
    L.append("    for (size_t i = 0; i < g_sceneCbs.size(); ++i) MMessage::removeCallback(g_sceneCbs[i]);")
...
def _generate_locator_cpp(spec, for_port=False):
    ...
    L.append("        if (tintFill) { dm.setColor(d.selColor); dm.mesh(MHWRender::MUIDrawManager::kTriangles, tri); }")
    L.append("        else if (perCorner) { dm.mesh(MHWRender::MUIDrawManager::kTriangles, tri, NULL, &tcol); }")
    L.append("        else { dm.setColor(pg.uniform); dm.mesh(MHWRender::MUIDrawManager::kTriangles, tri); }")
    ...
    L.append("            }")
    L.append("        }")
    L.append("        const MColor wcol = (selOK && hlWire) ? d.selColor : pg.wireColor;")
    L.append("        dm.beginDrawable();")
    ...
    L.append("    // line/point/text/shape draws in the selection color instead of its")
    L.append("    // own. Polygons decide per aspect, inside _drawPoly.")
