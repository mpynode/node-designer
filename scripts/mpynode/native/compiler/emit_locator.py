...
def _locator_hover_service(cls, data_cls, svars, needs_hover=True):
    ...
    if needs_hover:
        ...
        L.append("static double        g_lastTickT  = -1.0;  // wall time of the previous _poll tick (-1 = never)")
        L.append("static double        g_lastDirtyT = -1.0;  // wall time of the last idle-redraw request (-1 = never)")
        L.append("static double        g_lastRedraw = 0.0;   // redraw cost (s) behind the current wait")
        L.append("static double        g_pendingMax = 0.0;   // worst late tick gap since the last request")
    ...
    if needs_hover:
        ...
        L.append("    // keeps animating after the cursor leaves the gizmo), THROTTLED: a")
        L.append("    // 33 ms tick that arrives late was held up by a redraw, and the gap IS")
        L.append("    // that redraw's cost (the timer fires only once the main thread is")
        L.append("    // free again). Every tick is measured and the worst late gap since our")
        L.append("    // last request is remembered -- Maya may run the tick due right after")
        L.append("    // a request before OR after the redraw it triggered, and only one of")
        L.append("    // the two carries the cost. At the next request that gap becomes the")
        L.append("    // cost and the following request waits 2x it: idle animation takes at")
        L.append("    // most ~half the main thread however many locators are in the scene")
        L.append("    // (unthrottled, 100 nodes saturated it at 7.8 fps forever); no late")
        L.append("    // tick means a cheap redraw and the full tick rate. The late threshold")
        L.append("    // is 2 periods because under any redraw load Maya fires this timer")
        L.append("    // every ~47 ms, not 33. Port of draw_refresh.throttle_step.")
        L.append("    const double _now = _wallClock(), _period = 1.0 / 30.0;")
        L.append("    const double _gap = (g_lastTickT >= 0.0) ? (_now - g_lastTickT) : _period;")
        L.append("    g_lastTickT = _now;")
        L.append("    if (_gap > 2.0 * _period) g_pendingMax = std::max(g_pendingMax, _gap);")
        ...
        L.append("        g_lastRedraw = g_pendingMax; g_pendingMax = 0.0; g_lastDirtyT = _now;")
...
def _generate_locator_cpp(spec, for_port=False):
    ...
    L.append("    double wallClock = 0.0;   // time.time(); 0 for deterministic parity")
    L.append("    bool   selected = false;")
    ...
    if needs_hover:
        ...
        L.append("    inp.wallClock = _wallClock();          // monotonic seconds (tween deltas)")
