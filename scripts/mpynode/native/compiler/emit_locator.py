...
def _locator_hover_service(cls, data_cls, svars, needs_hover=True):
    ...
    if needs_hover:
        ...
        L.append("    // keeps animating after the cursor leaves the gizmo)")
...
def _generate_locator_cpp(spec, for_port=False):
    ...
    L.append("    std::vector<double>   textSize;   // raw size value (cast to unsigned at draw)")
    ...
    L.append("                dm.line(a, b); dm.endDrawable();")
    L.append("            } break;")
    L.append("            case 1: {  // point")
    L.append("                dm.beginDrawable();")
    L.append("                dm.setColor(hlAll ? d->selColor : d->pointColor[i]);")
    L.append("                dm.setPointSize(d->pointSize[i]); dm.point(d->pointPos[i]);")
    L.append("                dm.endDrawable();")
    L.append("            } break;")
    L.append("            case 2: {  // text")
    L.append("                dm.beginDrawable();")
    L.append("                dm.setColor(hlAll ? d->selColor : d->textColor[i]);")
    L.append("                dm.setFontSize((unsigned)d->textSize[i]);")
    L.append("                dm.text(d->textPos[i], d->textStr[i]); dm.endDrawable();")
    L.append("            } break;")
    ...
    L.append("                dm.endDrawable();")
    L.append("            } break;")
    L.append("            default: _drawPoly(dm, *d, d->polys[i], wInv); break;  // polygons")
    L.append("        }")
    L.append("    }")
    L.append("}")
    L.append("")
