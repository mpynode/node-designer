...
def _locator_hover_service(cls, data_cls, svars, needs_hover=True):
    ...
    if needs_hover:
        ...
        L.append("static double        g_lastRedraw = 0.0;   // measured cost of the last requested redraw (s)")
        L.append("static bool          g_armed      = false; // a request is out: the next tick measures it")
    ...
    if needs_hover:
        ...
        L.append("    // keeps animating after the cursor leaves the gizmo), THROTTLED: the")
        L.append("    // FIRST 33 ms tick after our own request arrives late by however long")
        L.append("    // the main thread spent on that redraw (the timer can only fire once")
        L.append("    // the thread is free again), so its lateness IS the redraw's cost; an")
        L.append("    // on-time tick means the redraw was cheaper than a tick. Wait 2x the")
        L.append("    // measured cost before the next request: idle animation takes at most")
        L.append("    // ~half the main thread however many locators are in the scene (an")
        L.append("    // unthrottled poll saturated it at 100 nodes: 7.8 fps forever), and a")
        L.append("    // cheap redraw keeps the full 30 fps. Port of draw_refresh.throttle_step")
        L.append("    // (late factor 1.25, backoff 2).")
        ...
        L.append("    if (g_armed) { g_lastRedraw = (_gap > 1.25 * _period) ? _gap : 0.0; g_armed = false; }")
        ...
        L.append("        g_lastDirtyT = _now; g_armed = true;")
